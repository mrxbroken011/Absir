...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        ...
        return await ex.edit("**Okay But Failed Because this user is in sudos.. 🐽**")
    elif user.id == DEVS:
        return await ex.edit("**You cannot raid a dev.. 🐽**")
...
@Client.on_message(
    filters.command(["rerstop"], ".") & (filters.me | filters.user(SUDO_USER))
)
async def stop_replyraid(client: Client, message: Message):
    args = await extract_user(message)
    reply = message.reply_to_message
    ex = await message.edit_text("`Processing...`")
    if args:
